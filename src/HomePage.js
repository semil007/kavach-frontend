...
const Home = () => {
  ...
  useEffect(() => {
    ...
    if (storedLoginTime) {
      ...
      console.log('User logged in at:', new Date(storedLoginTime).toLocaleString());
    }
  }, []);
  ...
  const handleLogout = () => {
    ...
    // Log the logout time and login time when logging out
    console.log('User logged out. Login Time was:', new Date(loginTime).toLocaleString());
